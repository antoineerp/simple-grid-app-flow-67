...
try {
    ...
    error_log("Chargement des groupes de collaboration pour l'utilisateur: {$userId} depuis l'appareil: {$deviceId}");
    ...
    if ($tableExists) {
        ...
        foreach ($groups as &$group) {
            // Convertir les dates
            if (isset($group['date_creation']) && $group['date_creation']) {
                $group['date_creation'] = date('Y-m-d\TH:i:s', strtotime($group['date_creation']));
            }
            if (isset($group['date_modification']) && $group['date_modification']) {
                $group['date_modification'] = date('Y-m-d\TH:i:s', strtotime($group['date_modification']));
            }
