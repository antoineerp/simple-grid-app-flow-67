...
<?php
/**
 * Modèle standard d'endpoint de synchronisation
 * 
 * Ce template peut être utilisé comme base pour créer de nouveaux endpoints
 * de synchronisation cohérents. Pour utiliser ce template:
 * 1. Copiez ce fichier vers votre nouvel endpoint
 * 2. Remplacez [TABLE_NAME] par le nom de votre table
 * 3. Ajustez les fonctions et les requêtes selon les besoins spécifiques
 */
...
try {
    ...
    error_log("Chargement de {$tableName} pour l'utilisateur: {$userId} depuis l'appareil: {$deviceId}");
    ...
    if ($tableExists) {
        ...
        foreach ($records as &$record) {
            // Convertir les dates
            if (isset($record['date_creation']) && $record['date_creation']) {
                $record['date_creation'] = date('Y-m-d\TH:i:s', strtotime($record['date_creation']));
            }
            if (isset($record['date_modification']) && $record['date_modification']) {
                $record['date_modification'] = date('Y-m-d\TH:i:s', strtotime($record['date_modification']));
            }
            
            // S'assurer que chaque enregistrement a un userId
            if (!isset($record['userId'])) {
                $record['userId'] = $userId;
            }
            
            // Ajouter ici d'autres conversions spécifiques à votre table
            // Par exemple: conversion de JSON, booléens, etc.
        }
    } else {
        ...
        $createTableSQL = "CREATE TABLE IF NOT EXISTS `{$userTableName}` (
            ...
            /* Ajoutez ici d'autres colonnes spécifiques à votre table */
    ...
    error_log("Enregistrements récupérés: " . count($records));
    echo json_encode([
        'success' => true,
        'records' => $records,
        'count' => count($records),
