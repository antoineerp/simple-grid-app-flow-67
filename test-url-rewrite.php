...
<?php
// Vérification complète des chemins assets Infomaniak
echo '<h2>Test des chemins d\'assets spécifiques à Infomaniak</h2>';

// Utiliser la fonction adjustPathForInfomaniak si elle existe
if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/api/config/env.php')) {
    ...
    if (function_exists('adjustPathForInfomaniak')) {
        ...
        // Tester la détection d\'environnement Infomaniak
        echo '<h3>Détection d\'environnement Infomaniak</h3>';
        echo '<ul>';
        echo '<li>IS_INFOMANIAK: ' . (env('IS_INFOMANIAK') ?? 'non défini') . '</li>';
        if (env('IS_INFOMANIAK') === 'true') {
            echo '<li class="success">Environnement Infomaniak correctement détecté</li>';
        } else {
            echo '<li class="warning">Environnement Infomaniak non détecté</li>';
        }
        echo '</ul>';
        
        // Tester les chemins d'assets spécifiques
        $assetPaths = [
            '/assets/index.js',
            '/assets/main.js',
            '/src/main.js',
        ...
        echo '<h3>Test des chemins d\'assets avec adjustPathForInfomaniak</h3>';
        echo '<table>';
        echo '<tr><th>Chemin d\'origine</th><th>Chemin ajusté</th><th>Existe?</th></tr>';
        
        foreach ($assetPaths as $path) {
            $adjustedPath = adjustPathForInfomaniak($path);
            $fullPath = $_SERVER['DOCUMENT_ROOT'] . $adjustedPath;
            $exists = file_exists($fullPath);
            
            echo '<tr>';
            echo '<td>' . htmlspecialchars($path) . '</td>';
            echo '<td>' . htmlspecialchars($adjustedPath) . '</td>';
        ...
        echo '<p class="error">Fonction adjustPathForInfomaniak non disponible</p>';
    }
} else {
