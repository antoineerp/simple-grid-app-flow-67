...
export const executeSyncOperation = async <T>(
  ...
  // Ajouter la tâche à la file d'attente au lieu d'exécuter immédiatement
  return syncQueue.enqueue(tableName, async () => {
...
// Vérifier si une synchronisation est en cours pour une table
export const isSynchronizing = (tableName: string): boolean => {
  return syncQueue.hasPendingTasks(tableName);
};

// Annuler les synchronisations en attente pour une table
